 ...
 */
public class DynamicHyperstereoscopySceneController extends BasicSceneController implements StereoSceneController
{
    /**
    ...
    public DynamicHyperstereoscopySceneController()
    {
...
//        System.out.println(dc.getView().getEyePosition()+ " " + dc.getView().getHeading() + " " + dc.getView().getPitch());
        ...
        //move the view to the left eye an render
		((TestFlyView) dcView).applyWithOffset(dc,leftEyeOffsetDir,hyperStereoOffset);
        super.draw(dc);

        // Move the view to the right eye
        Angle viewHeading = dcView.getHeading();
        ((TestFlyView) dcView).applyWithOffset(dc,rightEyeOffsetDir,hyperStereoOffset);
